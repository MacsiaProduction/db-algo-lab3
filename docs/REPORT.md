...
- **Поиск с высоким Recall@100 (≥ 0.95)** → **HNSW** `M=16, efConstruction=200, efSearch=80`: 10,460 QPS, 0.095 мс средняя latency, 3.88 ГБ на диске, 10.61 ГБ peak RSS (~58 % — transient overhead сборки, освобождается после `commit`/возврата). Для Recall@100 ≥ 0.99 — `M=16, efConstruction=200, efSearch=160` (R@100 = 0.9909, 6,286 QPS).
